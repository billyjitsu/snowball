...
interface AttackSummaryProps {
  attackResult?: number;
  setAttackComplete: React.Dispatch<React.SetStateAction<boolean>>;
  damageAmount?: bigint;
}

const AttackSummary: React.FC<AttackSummaryProps> = ({ attackResult, damageAmount, setAttackComplete }) => {
      ...
      <div className={`max-w-xs bg-${attackResult === 1 ? "black" : "green-500"} text-sm text-white rounded-md shadow-lg dark:bg-${attackResult === 1 ? "black" : "green-900"}`} role="alert">
        <div className="flex items-center p-4">
          ...
          <div className="ml-4">
            <button
              type="button"
              onClick={() => setAttackComplete(false)}
