...
function Arena() {
  ...
  const getTimeLeftForCurrentGame = async () => {
    try {
      // Retrieve the time value from the contract
      const time = await readContract({
        address: process.env.NEXT_PUBLIC_CONTRACT_ADDRESS as `0x${string}`,
        abi: Snowfight.abi,
        functionName: "getTimeLeftForCurrentGame",
      });

      const seconds = Number(time); // Convert BigInt to a regular number
      const days = Math.floor(seconds / 86400);

      // Update the state or perform other actions with the hours value
      setTimeLeft(days);
    } catch (error) {
      console.error("Error fetching time left for current game:", error);
    }
  }
      ...
      {minted && (
        ...
        >
          <div className="flex p-4">
            <span>Successful Mint. Click to close.</span>
...
export default dynamic(() => Promise.resolve(Arena), { ssr: false });
